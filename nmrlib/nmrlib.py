...
CC_xpeaks = [['C', 'CA'], ['CA', 'C'], # GENERAL       
            ['CA', 'CB'], ['CB', 'CA'],
            ['CB', 'CG'], ['CG', 'CB'],
            ['CG', 'CD'], ['CD', 'CG'],
            ['CD', 'CE'], ['CE', 'CD'],
            ['CG', 'CD1'], ['CD1', 'CG'], # TYR, PHE, THR, etc. 
            ['CG', 'CD2'], ['CD2', 'CG'],
            ['CD1', 'CE1'], ['CE1', 'CD1'],
            ['CD2', 'CE2'], ['CE2', 'CD2'],
            ['CE1', 'CZ'], ['CZ', 'CE1'],
...
class BrukerData:
   ...
   * pdir : string
   ...
   def __init__(self, pdir, rdir):
      ...
      self.pdic, self.data = ng.fileio.bruker.read_pdata(pdir, \
         scale_data = True)
      self.dic, _ = ng.fileio.bruker.read(rdir)
      self.udic = ng.fileio.bruker.guess_udic(self.dic, self.data, \
         strip_fake=False)
   ...
   def describe(self):
      ...
      if len(self.data.shape) == 1:
         ...
         print(f"The largest value is {np.amax(self.data)} and the smallest is {np.amin(self.data)}")
      ...
      * clevels : list (optional)
      ...
      if scale:
         ...
         for dim in np.arange(0, self.udic["ndim"]):
            ...
            self.udic[dim]["car"] = (self.dic['acqus']['SFO1'] - self.udic[dim]["obs"]) * 1e6
         ...
         mycnts = plt.contour(xgrid, ygrid, self.data, levels =
           clevels)
...
class BMRBData:
   ...
   * dir : int
   ...
   def __init__(self, id):
      ...
      self.cs = []
      for chemical_shift_loop in self.nmrstar.get_loops_by_category("Atom_chem_shift"):
         self.cs.append(chemical_shift_loop.get_tag(['Comp_index_ID', 
                                                     'Comp_ID', 
                                                     'Atom_ID', 
                                                     'Val']))
   ...
   def get_xpeak_df(self):
      ...
      xpeak_df = pd.DataFrame(columns = ['Index', 'Residue', 'Direct', 
                                       'Direct_CS', 'Indirect', 'Indirect_CS'])
      ...
      for residue in gcsdf:
         ...
         for cors in CC_xpeaks:
            ...
            if direct_atom in residue[1].ATOM_NAME.values \
            and indirect_atom in residue[1].ATOM_NAME.values:
               ...
               x_coord = residue[1][residue[1]['ATOM_NAME']==direct_atom]['CS'].item()
               y_coord = residue[1][residue[1]['ATOM_NAME']==indirect_atom]['CS'].item()
                       
               xpeak_df = xpeak_df.append({'Index' : index,
                                           'Residue' : residue_type,
                                           'Direct' : greeks[direct_atom],
                                           'Direct_CS' : x_coord,
                                           'Indirect' : greeks[indirect_atom],
                                           'Indirect_CS' : y_coord}, ignore_index = True)
